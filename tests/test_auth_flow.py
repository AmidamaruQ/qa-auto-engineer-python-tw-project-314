...
@pytest.mark.step_3
def test_burger_menu_and_dashboard(driver, base_url, logged_in_pages):
    driver.get(base_url)

    assert logged_in_pages.login.is_dashboard_visible(), (
        "Dashboard is not visible"
    )
    assert logged_in_pages.login.is_menu_open(), (
        "Side menu should be open after login"
    )

    logged_in_pages.login.toggle_menu()
    assert logged_in_pages.login.is_menu_closed(), (
        "Burger did not close menu"
    )

    logged_in_pages.login.toggle_menu()
    assert logged_in_pages.login.is_menu_open(), (
        "Burger did not reopen menu"
    )
    assert logged_in_pages.login.is_dashboard_visible(), (
...
@pytest.mark.step_3
def test_theme_toggle(driver, base_url, logged_in_pages):
    driver.get(base_url)

    initial_icon = logged_in_pages.login.get_theme_icon_testid()

    logged_in_pages.login.toggle_theme()
    logged_in_pages.login.wait_for_theme_icon_change(initial_icon)
    toggled_icon = logged_in_pages.login.get_theme_icon_testid()
    ...
    logged_in_pages.login.toggle_theme()
    logged_in_pages.login.wait_for_theme_icon_change(toggled_icon)

    assert logged_in_pages.login.get_theme_icon_testid() == initial_icon, (
